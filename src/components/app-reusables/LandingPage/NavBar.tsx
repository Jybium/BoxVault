            ...
            <nav className='text-white text-sm h-full'>
                <ul className="flex flex-col flex-1 justify-between h-full">
                    <div className="flex flex-col items-center gap-3">
                        ...
                        {firstPart.map((item) => (
                            <li key={item.id} className={item.id.toString()}>
                                <Link href={item.to} className={`flex flex-col items-center text-center gap-1 px-4 py-1 rounded-lg ${currentPath.includes(`${item.to}`) ? 'bg-gold text-black' : ''
                                    }`}>
                    ...
                    <div className="flex flex-col items-center gap-4">
                        ...
                        {secondPart.map((item) => (
                            <li key={item.id} className={item.id.toString()}>
                                <Link href={item.to} className={`flex flex-col items-center text-center gap-1 py-1 px-4 rounded-lg ${currentPath.includes(`${item.to}`) ? 'bg-gold text-black' : ''
                                    }`} >
